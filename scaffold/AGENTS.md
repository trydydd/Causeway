...
## Running your app locally (run-and-see)

Once `cwy validate` passes, run the app to see it work. The platform builds
the container for you — you still never write a Dockerfile.

```sh
cwy up          # build the app into a container and run it on localhost
```

`cwy up` validates the manifest, builds the image from a Dockerfile the
platform generates, starts the container, waits for your `healthEndpoint` to
return 200, and prints the URL (e.g. `→ http://localhost:8080`). Open it and
use the app — this behavioural check is the real verification, not a code
review.

Other commands in the loop:

```sh
cwy build               # build the image only
cwy build --show-dockerfile   # see the Dockerfile the platform would build (read-only)
cwy run                 # run an already-built image (builds first if needed)
cwy up -d               # run in the background instead of streaming logs
cwy down                # stop and remove the running container
```

All of these take an optional path to the project directory; with no path they
use the current directory.

**Project layout.** A Causeway project is just a directory containing:

- `causeway.yaml` — the manifest
- your entrypoint source (e.g. `app.py` / `server.js`, matching `entrypoint`)
- *(optional)* a dependency file the platform installs from — `requirements.txt`
  (Python) or `package.json` (Node)
- *(optional)* `.causeway.env` — local `KEY=VALUE` values for the variables you
  declared in `envVars`, used only for local runs

Write your application code however you like — there is no required framework or
structure. Two rules the loop depends on:

1. **Bind to `0.0.0.0`**, not `127.0.0.1`, on the `port` you declared — otherwise
   the platform cannot reach your app inside the container.
2. **Serve your `healthEndpoint`** so it returns HTTP 200 when ready.

**Local environment variables.** Put local values in `.causeway.env` — never in
source, the manifest, or any committed file (that is a denied policy violation).
The file is local-only and is never copied into the image; the platform injects
real values at runtime in higher tiers.

```sh
# .causeway.env  (local only — do not commit; add it to .gitignore)
DATABASE_URL=sqlite:///./local.db
```

**Note:** local `cwy up` is for *seeing your app run*. It is not deployment —
promotion to a shared tier always goes through the platform's CI gate, never
from your laptop.
