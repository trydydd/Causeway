...
| D38 | **`cwy init [NAME]` scaffolds a new project — manifest + agent rules only, never application code.** Creates `./NAME/` (or the current dir) with `causeway.yaml` (name pre-filled from the dir), `AGENTS.md`, and `.cursorrules`; refuses to overwrite an existing manifest. The scaffold files are bundled into the package by `docs/generate.py` and loaded via importlib.resources, same as the schema (D9) — the installed CLI can't see the repo's scaffold/. | First-touch friction is an adoption leak (PLANNING §2): the manual `mkdir` + copy-the-scaffold step was the largest friction in the dogfood run. `init` removes it while preserving the blank-slate principle (D37) — it lays down guidance, not code, so the agent still writes the app from scratch. Bundling-not-copying keeps the single-source-of-truth discipline (one canonical scaffold, drift-guarded by CI + a test). | Scaffolding an app stub too (fights D37's blank slate); telling users to mkdir + cp by hand (the friction that motivated this) |
