...
| D38 | **`cwy init [NAME]` scaffolds a new project — manifest + agent rules only, never application code.** Creates `./NAME/` (or the current dir) with `causeway.yaml` (name pre-filled from the dir), `AGENTS.md`, and the tool rule files; refuses to overwrite an existing manifest. Only the genuine sources (`causeway.yaml`, `AGENTS.md`) are bundled into the package (importlib.resources, same as the schema, D9); the rule files are *derived* at init (see D39). | First-touch friction is an adoption leak (PLANNING §2): the manual `mkdir` + copy-the-scaffold step was the largest friction in the dogfood run. `init` removes it while preserving the blank-slate principle (D37) — it lays down guidance, not code, so the agent still writes the app from scratch. | Scaffolding an app stub too (fights D37's blank slate); telling users to mkdir + cp by hand (the friction that motivated this) |
| D39 | **Tool rule files are *derived* at init from a single, extensible registry; not bundled.** `causeway/rules.py` holds `RULE_FILES` (Cursor + Copilot today) and the `AGENTS.md`→rule-file transform. Both `cwy init` (runtime) and `scripts/generate.py` (build time) use it, so the repo's `scaffold/` rule files and the files `cwy init` writes are identical by construction. Adding a host is one registry entry. The generator lives in `scripts/`, not `docs/` (no code in a docs dir). | Research showed `AGENTS.md` is now natively read by the target hosts (Cursor, Copilot), so the per-tool files are compatibility shims, not load-bearing (D10/D11). Bundling a derived copy alongside its own source was pure redundancy — it forced a dotfile-packaging hack and left the Copilot shim asymmetric (init never shipped it). Deriving from one registry removes the redundancy, fixes the asymmetry, drops the hack, and makes future hosts trivial to add — all in service of minimal maintenance surface (no-staff). | Bundling pre-derived rule files (redundant, drift-prone, dotfile hack); dropping the shims entirely (bets on unverified M365-web native support); leaving generator code in `docs/` |
