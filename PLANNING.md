...
- "Business user" is not one population. Provide more than one front door (IDE for the technical-enough, a thinner surface for the rest); don't force everyone through the IDE.
...
| D10 | **Manifest authored in YAML with `# yaml-language-server: $schema=` header**, enum-heavy, LLM-tuned `description`/`examples` | Free inline validation + autocomplete in Cursor/Copilot with zero setup; enums stop the agent inventing values; descriptions are what the model conditions on | Free-text-heavy schema; no editor integration |
| D11 | **Agent instruction files in the scaffold** (AGENTS.md + tool-specific rules) stated as *forbidden actions* | The agent's training data is saturated with the patterns we forbid (Dockerfiles, compose, hardcoded secrets); forbidding + a pre-filled scaffold makes the safe path the path of least resistance | Rely on the agent's defaults |
